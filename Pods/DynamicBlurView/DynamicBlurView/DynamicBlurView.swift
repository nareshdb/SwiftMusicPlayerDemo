...
open class DynamicBlurView: UIView {
    ...
    open override func didMoveToSuperview() {
     ...
     func async(on queue: DispatchQueue, actions: @escaping () -> Void) {
     ...
     func sync(on queue: DispatchQueue, actions: () -> Void) {
        if drawsAsynchronously {
     ...
     func draw(_ image: UIImage, blurRadius radius: CGFloat, fixes isFixes: Bool, baseLayer: CALayer?) {
     ...
     func blurLayerRect(to layer: CALayer, conversion: Bool) -> CGRect {
        if conversion {
     ...
     func snapshotImage(for layer: CALayer, conversion: Bool) -> UIImage? {
